...
## 次の作業（候補 / 優先度順）
1. `--filter` オプションの実処理実装（名前空間パターンによる結果絞り込み）
2. 循環依存検出（DFS で最小実装）
3. 例外・警告出力の整理（メッセージ/終了コード方針の統一）
4. `--help` / `--version` オプション追加
5. 出力 JSON スキーマの固定化（仕様化と互換ルール定義）
6. `examples/` に最小E2Eサンプルを常設（README手順の再現性向上）
